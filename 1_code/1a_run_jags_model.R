...
cat(file="1_code/jags_model.txt","
model{
...
sd.alpha0 ~ dt(0, 1/2.25^2, 1) T(0,) # SD hyperparameter
...
for(o in 1:nobservers){
  ...
  sd.beta1[o] ~ dt(0, 1/2.25^2, 1) T(0,) # SD hyperparameters
...
# estimated number of occurrences per species
for(k in 1:nspecies){
  n.occ.est[k] <- sum(z[,k])
}

# estimated species richness per site
for(i in 1:nsites){
  N.est[i] <- sum(z[i,])
}

# species-specific occupancy and species-specific detection
for(k in 1:nspecies){
  logit(occ.pred[k]) <- alpha0[k] + alpha1*1
}



# ## Assess model fit through Bayesian p-values at species-level (conditional on fitted random effects)
# 
# for(k in 1:nspecies){
#   for(i in 1:nsites){
#     help.y[k,i] <- mean(y[,1,k])
#     help.y.sim[k,i] <- mean(y.sim[,1,k])
# 
#     help.cv.y[k,i] <- sd(y[,1,k])/max(0.001, mean(y[,1,k]))
#     help.cv.y.sim[k,i] <- sd(y.sim[,1,k])/max(0.001, mean(y.sim[,1,k]))
#   }
# 
#   for(j in (nsites+1):(nsites+46)){
#     help.y[k,j] <- mean(y[index,2,k])
#     help.y.sim[k,j] <- mean(y.sim[index,2,k])
# 
#     help.cv.y[k,j] <- sd(y[index,2,k])/max(0.001, mean(y[index,2,k]))
#     help.cv.y.sim[k,j] <- sd(y.sim[index,2,k])/max(0.001, mean(y.sim[index,2,k]))
#   }
# 
#  mean.y[k] <- mean(help.y[k,])
#  mean.y.sim[k] <- mean(help.y.sim[k,])
#  p.mean[k] <- step(mean.y.sim[k] - mean.y[k])     # Bayesian p-value mean
# 
#  cv.y[k] <- mean(help.cv.y[k,])
#  cv.y.sim[k] <- mean(help.cv.y.sim[k,])
#  p.cv[k] <- step(cv.y.sim[k] - cv.y[k])           # Bayesian p-value sd
# }
...
params <- c("mu.alpha0","sd.alpha0",
            ...
            ,"N.est","n.occ.est"     # species richness and number of occurrences per species
            
            ,"occ.pred"              # average occupancy
            ,"beta1"

            # ,"p.mean","p.cv"       # assess goodness-of-fit
            )

# MCMC settings
na=2 ;  nc=1 ;  ni=4 ;  nb=2 ;  nt=1 # try-out: 20 min
na=6000 ;  nc=5 ;  ni=50000 ;  nb=30000 ;  nt=20 # takes approximately 3.3 days!
paste(nc*(ni-nb)/nt, "posterior samples")


# Call JAGS
output <- jags(data, inits, params, "1_code/jags_model.txt", n.chains=nc,
               n.thin=nt, n.iter=ni, n.burnin=nb, parallel=ifelse(nc>1,TRUE,FALSE))
save(output, file="2_output/output.jags_model.RData")
