...
cat(file="1_code/jags_model_priors1.txt","
model{
...
alpha1 ~ dnorm(0, 0.01)          # parameter for coefficient of substrate

mu.alpha2 ~ dnorm(0, 0.01)        # Mean hyperparameter for coefficient of altitude.st
sd.alpha2 ~ dt(0, 1/2.25^2, 1) T(0,) # SD hyperparameter
tau.alpha2 <- 1/sd.alpha2^2

mu.alpha3 ~ dnorm(0, 0.01)        # Mean hyperparameter for coefficient of altitude.st .2
sd.alpha3 ~ dt(0, 1/2.25^2, 1) T(0,) # SD hyperparameter
tau.alpha3 <- 1/sd.alpha3^2

mu.alpha4 ~ dnorm(0, 0.01)        # Mean hyperparameter for coefficient of precipiation.st
sd.alpha4 ~ dt(0, 1/2.25^2, 1) T(0,) # SD hyperparameter
tau.alpha4 <- 1/sd.alpha4^2

mu.alpha5 ~ dnorm(0, 0.01)        # Mean hyperparameter for coefficient of precipitation.st.2
sd.alpha5 ~ dt(0, 1/2.25^2, 1) T(0,) # SD hyperparameter
tau.alpha5 <- 1/sd.alpha5^2

mu.alpha6 ~ dnorm(0, 0.01)        # Mean hyperparameter for coefficient of solar.st
sd.alpha6 ~ dt(0, 1/2.25^2, 1) T(0,) # SD hyperparameter
tau.alpha6 <- 1/sd.alpha6^2

mu.alpha7 ~ dnorm(0, 0.01)        # Mean hyperparameter for coefficient of solar.st.2
...
beta2 ~ dnorm(0, 0.01)     # prior for coefficient of conspicuousness
beta3 ~ dnorm(0, 0.01)     # prior for coefficient of identifiability
beta4 ~ dnorm(0, 0.01)     # prior for coefficient of experience
...
# Call JAGS
output <- jags(data, inits, params, "1_code/jags_model_priors1.txt", n.chains=nc,
               n.thin=nt, n.iter=ni, n.burnin=nb, parallel=ifelse(nc>1,TRUE,FALSE))
save(output, file="2_output/output.jags_model_priors1.RData")
